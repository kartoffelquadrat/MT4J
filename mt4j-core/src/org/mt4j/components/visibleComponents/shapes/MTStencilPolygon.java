...
//	    	GLStencilUtil.getInstance().beginDrawClipShape(gl);
	    	...
	    	if (GLStencilUtil.getInstance().isClipActive() && gl != null){
...
//			if (GLStencilUtil.getInstance().isClipActive()){
				gl.glStencilFunc(GL2.GL_NOTEQUAL, GLStencilUtil.stencilValueStack.peek(), GLStencilUtil.stencilValueStack.peek());
...
//				gl.glVertex3d (minX, maxY, 0.0); 
				...
				gl.glVertexPointer(3, GL2.GL_FLOAT, 0, stencilQuad);
		    	gl.glDrawArrays(GL2.GL_TRIANGLE_FAN, 0, stencilQuad.capacity()/3); 
		    	...
		    	if (GLStencilUtil.getInstance().isClipActive()){
		    		if (gl != null){
		    			...
		    			gl.glStencilFunc(GL2.GL_EQUAL, GLStencilUtil.stencilValueStack.peek(), GLStencilUtil.stencilValueStack.peek());
...
//			}
		    ...
		    for (FloatBuffer outline : contoursInfos){
		    	gl.glVertexPointer(3, GL2.GL_FLOAT, 0, outline);
		    	gl.glDrawArrays(GL2.GL_LINE_STRIP, 0, outline.capacity()/3); 
		    }
...
//		    gl.glStencilOp(GL.GL_KEEP, GL.GL_INVERT, GL.GL_INVERT);
			...
			if (indexBuff == null){
				...
				gl.glDrawElements(GL2.GL_TRIANGLE_FAN, indexBuff.capacity(), GL2.GL_UNSIGNED_SHORT, indexBuff);
			}
